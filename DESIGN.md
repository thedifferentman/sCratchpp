...
| D09 | 一个绿旗启动脚本初始化后调用 `main`。可通过资源清单生成滚轮与键盘采集帽子，帽子仅向原生列表入队；所有 LLVM 回调由主执行上下文显式轮询调用。 |
| D10 | 单执行角色 Program，无克隆；支持画笔、造型显示及图章。所有资源包共享该角色。SB3 格式所需的舞台不算第二个执行角色。 |
...
| D21 | 控制台先提供独立的画笔输出与键盘行输入接口，再供后续标准流适配复用；不自动绑定 iostream 或 stdio。数学函数可由 LLVM 运行库实现。 |
...
| D25 | 资源作为独立链接单元，由 sCrpp.toml 声明包名、PNG/SVG 路径、尺寸和中心。造型名称为包名::造型名，不生成编号绑定；运行时 set_string 将 UTF-8 写入 __scl_string，再由官方切换造型积木按名称处理。PNG 内嵌原像素至 SVG；首版全部打包、按最终文件内容去重，不进行资源裁剪。 |
...
- 当前单角色、无克隆的限制仍生效。事件帽子由受约束的清单生成，不能通过普通内联汇编插入；帽子不会进入 LLVM 栈或直接调用 LLVM 回调。
...
资源使用独立的 TOML 清单，准备工具输出自包含的 resources.json 和内容寻址的 SVG 文件，最终由 --resources 与代码一起打包。纯资源不进入 LLVM 字节内存，也不依赖 LLVM 元数据或符号可达性。

首版不裁剪资源，因此允许造型名称来自字面量或运行时字符串。LLVM 函数符号仍需通过可追踪的操作数或保留约定表达。构建生成的 scratch.cpp 实现薄包装；头文件只声明接口。

资源记录至少包含逻辑符号、类型、内容或构建输入引用，以及对应积木字段。打包阶段生成内容校验标识、合并重复内容并重写相关引用。资源封装仍是 SB3 打包工作，不是第二个代码链接器。

### 13.6 原生列表、事件与控制台库

基础 SDK 只提供 C++ 标准库和底层运行时。Console、Events、PTE 改为普通 scrate 包：项目用 `[dependencies]` 声明精确版本或本地路径，解析传递依赖并生成 `scrate.lock`，再链接对应源码/bitcode和资源。无依赖项目不携带这些库的资源。包名与 `package.resource_namespace` 分离，保留原生列表命名。默认静态仓库为 `https://scrate.shapy.cn`；客户端验证 SHA-256、固定锁文件来源、缓存和 ABI，支持离线构建，不执行包安装脚本。发布准备只生成本地索引和 ZIP，不上传。详见 `docs/SCRATE.md`。

`[[lists]]` 从 UTF-8 TXT 按 LF 初始化原生列表，保留空行和字形记录中的空格；名称使用 `包名::列表名`。只读表的写操作在生成积木时拒绝。`[library]` 声明源文件和包含路径，由构建工具编译为 LLVM 后统一链接，不把代码附着到资源文件中。

`include/events` 提供滚轮、键盘队列及显式 `poll/run`。滚轮方向键帽子调用不让出的原生采集过程，过滤当前仍按住的对应键后入队，不改动 LLVM 内存和栈。键盘的上/下箭头帽子使用相反过滤条件排除滚轮，其余键盘帽子保留普通键快速松开后的按下通知。键盘覆盖 ASCII 符号、字母、数字、空格、回车、四方向及 TW 额外键；原版字母折叠为大写，TW 根据采集时的 Shift 状态决定大小写。不合成无法观察的按键、松开事件或 IME 文本。两队列分别快照，先分派滚轮再分派键盘，不承诺跨队列总顺序。主程序返回后不再接收新事件。差分识别只能推断方向，不能保证快速松键或混合键盘/滚轮情况下准确，也不保证高频事件逐个保留。

`include/pte` 独立管理用户提供的 42,221 个字形与 BMP 数字码点索引，纯官方积木汇编绘字；字形内部不主动让出执行。`include/console` 依赖 PTE 和 events，维护有界文本历史，追加时仅绘制新增字符，覆盖或滚屏时清除画笔层重绘。控制台独占唯一角色的画笔层，无自动标准流重定向。

行输入先提供 `begin_input/try_read_line` 和阻塞便利函数 `read_line`，后续标准流适配复用输入缓冲，不另建第二套编辑器。当前仅收取 ASCII 键值；TW 在采集时按 Shift 状态区分字母大小写，使用 Backspace 删除，反斜杠是普通字符；原版字母大写，使用反斜杠删除。Enter 提交。`scratch::is_turbowarp()` 使用兼容的 `argument_reporter_boolean VALUE="is turbowarp?"` 判断环境，TW 解释/编译模式均返回真，原版返回假。大小写只检测 Shift，不处理 Caps Lock；Shift 在帽子执行前变化会影响结果。删除可以跨软换行，但不越过提示文字；输入长度受保留历史容量限制。外部输出取消活动输入并换行。事件回调中不允许开始阻塞读取或新输入会话，避免分派重入和同批 Enter 后字符进入新会话。

用户显式调用 `flush()`；它先处理排队事件，再在字形之间执行 timing refresh。刷新使用原生全局时间戳，天数差乘 86400000 达到 8 ms 后执行空思考零秒并等待，恢复后更新时间戳；空闲 flush 同样检查刷新。`double days_since_2000()` 通过汇编的精确 binary64 输出桥保留小数。此机制不会自动插入任意用户循环，也不会创建并行 LLVM 执行上下文。
