...
symbols), no threads, no filesystem, no random device, no locale, no wide-character
library, no timezone database, and serial parallel-algorithm fallback. Exceptions
and RTTI are disabled by compiler flags in the generated SDK manifest. The
...
inclusion; their presence is not a claim that every API is implemented. In
particular, I/O, filesystem, locale, threading, exception unwinding, RTTI,
floating-point string conversion, and the mathematical library are not part of
the first-stage runtime. Unsupported reachable external calls are diagnosed by
