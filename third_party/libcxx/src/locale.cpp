...
// Scratch guest: only the classic narrow-character facets are provided.
...
locale::__imp::__imp(const string& name, size_t refs) : facet(refs), facets_(N), name_("C") {
  if (name != "C" && name != "POSIX" && !name.empty())
    __throw_runtime_error("Scratch supports only the C locale");
  facets_ = locale::classic().__locale_->facets_;
  for (unsigned i = 0; i < facets_.size(); ++i)
    if (facets_[i]) facets_[i]->__add_shared();
}
